  ...
  <div class="p-6">
      ...
      <div class="flex justify-between items-center mb-6">
          <div>
              ...
              <p class="text-sm text-gray-500">{{ t('plans.subtitle') }}</p>
          </div>
      ...
      <div v-else class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          <table class="w-full text-left border-collapse">
              <thead>
                  <tr class="bg-gray-50 border-b text-gray-600">
                      <th class="p-4 font-bold text-xs uppercase tracking-wider">{{ t('table.col_code') }}</th>
                      <th class="p-4 font-bold text-xs uppercase tracking-wider">{{ t('table.col_name') }}</th>
                      <th class="p-4 font-bold text-xs uppercase tracking-wider">{{ t('table.col_tier') }}</th>
                      <th class="p-4 font-bold text-xs uppercase tracking-wider">{{ t('table.col_price') }}</th>
                      <th class="p-4 font-bold text-xs uppercase tracking-wider text-center">{{ t('table.col_duration') }}</th>
                      <th class="p-4 font-bold text-xs uppercase tracking-wider">{{ t('table.col_status') }}</th>
                      <th class="p-4 font-bold text-xs uppercase tracking-wider text-right">{{ t('table.col_actions') }}</th>
                  </tr>
              </thead>
              <tbody>
                  <tr v-for="plan in filteredPlans" :key="plan.id" 
                      ...
                      <td class="p-4 font-bold text-gray-800 text-sm">
                          {{ plan.name }}
                          <span v-if="plan.planType === 'FREE'" class="ml-2 text-[10px] text-blue-400 font-medium px-1.5 py-0.5 border border-blue-200 rounded uppercase">{{ t('plans.system_core') }}</span>
                          <span v-else-if="!plan.active" class="ml-2 text-[10px] text-red-400 font-medium px-1.5 py-0.5 border border-red-200 rounded uppercase">{{ t('plans.retired') }}</span>
                      </td>
                      ...
                      <td class="p-4 font-bold text-gray-700 text-sm">{{ formatCurrency(plan.price, plan.currency) }}</td>
                      <td class="p-4 text-center font-medium text-xs">{{ plan.durationMonths }} Mo</td>
                      <td class="p-4">
                          <div class="flex items-center gap-2">
                             <div :class="plan.active ? 'bg-green-500' : 'bg-red-400'" class="w-2 h-2 rounded-full"></div>
                             <span :class="plan.active ? 'text-green-700' : 'text-red-500'" class="text-[10px] font-black uppercase">
                                 {{ plan.active ? t('status.ACTIVE') : t('status.LOCKED') }}
                             </span>
                          </div>
                      </td>
                      <td class="p-4 text-right">
                          <div class="flex justify-end gap-3 items-center">
                              ...
                              <template v-if="plan.planType === 'FREE'">
                                  <span class="text-[10px] text-gray-400 font-bold uppercase italic pr-2">{{ t('plans.system_core') }}</span>
                              </template>
                              ...
                              <template v-else-if="plan.active">
                                  ...
                                  <button @click="deletePlan(plan)" 
                                          class="text-red-600 hover:text-red-800 text-[10px] font-bold border border-red-100 px-2 py-1 rounded hover:bg-red-50 uppercase shadow-sm">
                                      {{ t('plans.deactivate') }}
                                  </button>
                              </template>
                              ...
                              <template v-else>
                                  <button @click="handleActivate(plan)" 
                                          class="text-green-600 hover:text-green-800 text-[10px] font-bold border border-green-100 px-3 py-1 rounded hover:bg-green-50 uppercase shadow-sm">
                                      {{ t('plans.enable') }}
                                  </button>
                              </template>
                          </div>
          ...
          <div v-if="!filteredPlans.length" class="p-12 text-center text-gray-500 italic text-sm">
              {{ t('plans.no_plans') }}
          </div>
      </div>
